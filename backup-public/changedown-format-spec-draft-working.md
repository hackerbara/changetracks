...
| Substitution | `{~~old[^cn-32]`            | `REST`                         |[^cn-33]
| Highlight    | `text`                | `important`                             |[^cn-34][^cn-35]
| Comment      | ``                | ``                        |
...
: @alice | 2024-01-15 | sub | rejected
    ...
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Inherited proposal marker from copied file should not remain pending in the working draft."
...
|--------------|-----------------------------|----------------------------------|
| Insertion    | ``                | text is added                    |[^cn-38]
| Deletion     | `text`                | text is removed                  |[^cn-39]
| Substitution | `old`            | old is replaced with new         |[^cn-40]
| Highlight    | `text`                | text is unchanged (annotation)   |[^cn-41]
| Comment      | ``                | no body text (annotation only)   |

Highlights attach comments with no whitespace: `text`. All types support multi-line content. Substitutions use `~>` to separate old text from new.[^cn-43]
...
Inline CriticMarkup must not nest — `` is malformed. CriticMarkup appearing inside footnote discussion text is literal content, not parsed as markup.~>### Level 0 — Bare Markup[^cn-46]

CriticMarkup defines five inline constructs:

| Type         | Syntax              | Example                    |
|--------------|---------------------|----------------------------|
| Insertion    | ``        | ``         |[^cn-47][^cn-48]
| Deletion     | `text`        | `removed this`       |[^cn-49][^cn-50]
| Substitution | `old`    | `REST`      |[^cn-51][^cn-52]
| Highlight    | `text`        | `important`          |[^cn-53][^cn-54]
| Comment      | ``        | ``     |

Any markdown file containing these constructs is a valid CriticMarkup document. This is Level 0 — the substrate.
...
Attach an adjacent comment (no whitespace after the change delimiter), and metadata travels with the change:

```markdown
The API should use {~~REST~>GraphQL~~}{>>@alice | 2024-01-15 | sub | proposed<<} for the public interface.
```

The comment carries pipe-separated fields: `@author`, date, type, status. Same `|` separator at every level.

### Level 2 — Full Deliberation

Add a footnote reference plus a footnote definition block and the change gains a full deliberation record:

```markdown
The API should use {~~REST~>GraphQL~~}[^ex-1] for the public interface.

[^ex-1]: @alice | 2024-01-15 | sub | proposed
    @alice 2024-01-15: GraphQL reduces over-fetching for dashboard clients.
    @dave 2024-01-16: But increases client complexity.
      @alice 2024-01-16: See benchmarks in PR #42.
    resolved @dave 2024-01-17
...
The footnote header carries identity and status. The body carries discussion, resolution markers, approval records, and revision history.

### Level Transitions

Each level is a strict superset: Level 2 is valid Level 1 is valid Level 0. Promotion adds a container — L0 -> L1 adds a comment, L1 -> L2 adds a footnote.

Compaction moves in the opposite direction. It removes selected historical footnotes, inserts a compaction boundary at the retained frontier, and relies on version history to preserve the full prior deliberation record.~~}[^cn-31.1][^cn-1][^cn-1][^cn-21.1]

### Range Changes (Deferred)

Range-change syntax is intentionally out of scope for this draft.

This spec version focuses on the stable core: inline change types, identity, footnote structure, anchors, projection semantics, coherence, and stewardship. Range operations can be added in a follow-up revision once parser and conversion behavior are locked end-to-end with tests.[^cn-26.1][^cn-5][^cn-6][^cn-6][^cn-7][^cn-21.2]
---
...
[^cn-5]: @alice | 2024-03-01 | move | rejected
    note: Move intro from section 3 to section 1
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Dangling inherited proposal artifact in working copy; not part of intended spec edits."

[^cn-5.1]: @alice | 2024-03-01 | del | rejected
    3:b2 The intro paragraph here.
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Dangling inherited proposal artifact in working copy; not part of intended spec edits." (cascaded from cn-5)

[^cn-5.2]: @alice | 2024-03-01 | ins | rejected
...
The spec defines canonical producer output. Parsers should prefer compatibility over brittleness for reasonable variations.[^cn-26.2][^cn-20]
...
[^cn-1]: @alice | 2024-01-15 | sub | rejected
    @alice 2024-01-15: GraphQL reduces over-fetching for dashboard clients.
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Stale inherited proposal: footnote context documents it as dangling/cascaded artifact, not intended spec edit."
```
...
The CriticMarkup is settled into the body — the body now reads `GraphQL` as clean text. The edit-op line (`4:e2 should use GraphQL for the public`) is written as the coherence verification record.[^cn-57]
...
`Protocol ` is the context before. `new o` is the operation. `verview` is the context after. The combined string appears exactly once on line 3 — any parser can recover the operation's exact column position.[^cn-58]
...
For deletions (where the deleted text is absent from the body), the context is the text surrounding the deletion point: `contextBeforecontextAfter`. The joined context locates where the deletion occurred.[^cn-59]
...
| Rejected | Body remains clean with rejection applied | Keep edit-op line    |[^cn-26.5]
...
: @alice | 2024-01-15 | sub | accepted
    4:e2 should use GraphQL for the public`) serve as coherence verification records: any tool can check that line 4 still contains `should use GraphQL for the public` and know the footnote is correctly anchored.[^cn-60]

The proposed highlight (cn-3) has inline CriticMarkup in the body — `100 req/min`. It has a blocking issue and an open thread awaiting load test results.

Two resolved threads. One open. Any reader — human or machine — can follow the full state of deliberation from the file alone.
~>## Complete Example

A concise L2 snapshot with one decided change and one open deliberation thread.
{~~
```markdown
<!-- changedown.com/v1: tracked -->
# API Design Document
...
: @alice | 2024-01-15 | sub | accepted
    3:e2 should use {~~REST~>GraphQL~~} for the public interface.
    approved: @eve 2024-01-20 "Correct direction"
    resolved @eve 2024-01-20

: @carol | 2024-01-17 | highlight | rejected
    @carol 2024-01-17 [issue/blocking]: 100/min feels low for production.
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Inherited proposed highlight in example region should not remain as live proposal in the working draft."
    open -- awaiting load test results
```
[^cn-61]
This snapshot shows both surfaces clearly: decided text remains locally verifiable through edit-op records, while unresolved deliberation stays visible in-place.~~}
---~>## Complete Example
...
The API should use GraphQL for the public interface.
Rate limiting is set to {==100 req/min==}{>>needs load-test confirmation<<}[^ex-3].

[^ex-1]: @alice | 2024-01-15 | sub | accepted
    3:e2 should use {~~REST~>GraphQL~~} for the public interface.
    approved: @eve 2024-01-20 "Correct direction"
    resolved @eve 2024-01-20

[^ex-3]: @carol | 2024-01-17 | highlight | proposed
    @carol 2024-01-17 [issue/blocking]: 100/min feels low for production.
    open -- awaiting load test results
```

This snapshot shows both surfaces clearly: decided text remains locally verifiable through edit-op records, while unresolved deliberation remains visible in place.~~}[^cn-31.3][^cn-3][^cn-1][^cn-3][^cn-27][^cn-3][^cn-1][^cn-2][^cn-3][^cn-3][^cn-19]
...
[^cn-25.1]: @ai:claude-opus-4.6 | 2026-04-02 | sub | accepted
    ...
    approved: @ai:gpt-5.3-codex 2026-04-02T02:12:37Z "Adopt canonical Current/Decided/Original projection language in this future-first draft."

[^cn-25.2]: @ai:claude-opus-4.6 | 2026-04-02 | sub | accepted
    ...
    approved: @ai:gpt-5.3-codex 2026-04-02T02:12:37Z "Align matching-cascade terminology with canonical projection vocabulary."

[^cn-25.3]: @ai:claude-opus-4.6 | 2026-04-02 | sub | accepted
    ...
    approved: @ai:gpt-5.3-codex 2026-04-02T02:12:37Z "Replace misleading accepted-all phrasing with Current projection semantics."

[^cn-25.4]: @ai:claude-opus-4.6 | 2026-04-02 | sub | accepted
    ...
    approved: @ai:gpt-5.3-codex 2026-04-02T02:12:37Z "Keep stewardship section terminology consistent with canonical projections."

[^cn-25]: @ai:claude-opus-4.6 | 2026-04-02 | group | proposed
    @ai:claude-opus-4.6 2026-04-02T00:14:40Z: propose_batch

[^cn-26.1]: @ai:gpt-5.3-codex | 2026-04-02 | sub | accepted
    @ai:gpt-5.3-codex 2026-04-02T02:12:14Z: Trim scope for this draft and avoid over-specifying a section you marked as potentially unnecessary right now.}
    approved: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Defer range syntax for this draft to reduce scope and avoid over-specifying unsettled behavior."

[^cn-26.2]: @ai:gpt-5.3-codex | 2026-04-02 | sub | accepted
    @ai:gpt-5.3-codex 2026-04-02T02:12:14Z: Soften strictness while preserving canonical producer intent and future-first migration flexibility.}
    approved: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Keeps canonical producer output while intentionally loosening parser strictness language."

[^cn-26.5]: @ai:gpt-5.3-codex | 2026-04-02 | sub | accepted
    @ai:gpt-5.3-codex 2026-04-02T02:12:14Z: Match your requested future-first target model rather than implementation caveats.}
    approved: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Aligns demotion section with future-first fully materialized semantics."

[^cn-26.7]: @ai:gpt-5.3-codex | 2026-04-02 | del | accepted
    @ai:gpt-5.3-codex 2026-04-02T02:12:14Z: Per your request, remove legacy-domain mention from public spec narrative.}
    approved: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Removes legacy header-domain mention per requested public-spec policy."

[^cn-26]: @ai:gpt-5.3-codex | 2026-04-02 | group | proposed
    @ai:gpt-5.3-codex 2026-04-02T02:12:14Z: propose_batch

[^cn-27]: @ai:gpt-5.3-codex | 2026-04-02 | sub | rejected
    @ai:gpt-5.3-codex 2026-04-02T02:12:59Z: Shorten the example while preserving the essential semantics and reducing repetitive thread detail.}
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Proposal shell exists without intended content due overlap failure; replacing with fresh explicit edit."

[^cn-28]: @ai:gpt-5.3-codex | 2026-04-02 | sub | rejected
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Malformed overlap artifact from failed large replacement; superseded by clean rewrite plan."

[^cn-29]: @ai:gpt-5.3-codex | 2026-04-02 | ins | rejected
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Accidental insertion artifact caused by overlap in tracked example block."

[^cn-30]: @ai:gpt-5.3-codex | 2026-04-02 | del | rejected
    rejected: @ai:gpt-5.3-codex 2026-04-02T02:15:22Z "Empty deletion artifact with no author/intent; remove from draft history."

[^cn-31.1]: @ai:gpt-5.3-codex | 2026-04-02 | sub | accepted
    @ai:gpt-5.3-codex 2026-04-02T02:16:34Z: Repair corrupted examples and keep this section concise but complete.}
    approved: @ai:subagent 2026-04-02T02:21:25Z "Removes spurious `~~}` and stray footnote refs from the compaction / L3→L2 demotion paragraph; restores readable stewardship semantics without garbled markup."

[^cn-31.3]: @ai:gpt-5.3-codex | 2026-04-02 | sub | accepted
    @ai:gpt-5.3-codex 2026-04-02T02:16:34Z: Shorten and de-noise the end example while preserving essential semantics.}
    approved: @ai:subagent 2026-04-02T02:21:25Z "Removes trailing `~~}` from the closing complete-example narrative before References; de-noises the snapshot paragraph as intended."

[^cn-31]: @ai:gpt-5.3-codex | 2026-04-02 | group | proposed
    @ai:gpt-5.3-codex 2026-04-02T02:16:34Z: propose_batch

[^cn-32]: @ai:subagent | 2026-04-02 | sub | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Garbled substitution: entire Level 0 heading/table is wrapped as sub with broken modified_text (new only); would corrupt the spec."

[^cn-33]: @ai:subagent | 2026-04-02 | sub | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Part of the same malformed duplicate-table edit session as cn-32; not a clean standalone edit."

[^cn-34]: @ai:subagent | 2026-04-02 | hig | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Downstream of corrupted Level 0 table block; rejects softer spec table in favor of inline markup noise."

[^cn-35]: @ai:subagent | 2026-04-02 | hig | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same as cn-34: noisy highlight proposals tied to the damaged table region."

[^cn-36]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Empty/noisy comment proposal with no substantive content."

[^cn-37]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Empty/noisy comment proposal with no substantive content."

[^cn-38]: @ai:subagent | 2026-04-02 | ins | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Inserts literal markup into the syntax table in a way that duplicates/conflicts with the intended concise table; part of malformed batch."

[^cn-39]: @ai:subagent | 2026-04-02 | del | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same malformed batch: delimiter markup embedded in cells without clear coherent spec goal."

[^cn-40]: @ai:subagent | 2026-04-02 | sub | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same batch: substitution markup in table cells adds noise versus intended direction."

[^cn-41]: @ai:subagent | 2026-04-02 | hig | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same batch: unnecessary highlight markup in syntax column."

[^cn-42]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Empty comment markup in table row; noisy."

[^cn-43]: @ai:subagent | 2026-04-02 | hig | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Merges highlight+comment into one cell string; garbled pedagogy."

[^cn-44]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Clearly malformed: comment markup contains prose fragments (`reason` annotation...) as if inside `{>>...<<}`."

[^cn-45]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Tied to line 89 garbled region; duplicate/noisy comment cell."

[^cn-46]: @ai:subagent | 2026-04-02 | ins | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Context shows a bad splice into duplicate Level 0 block (`~>### Level 0`); not a clean nest example."

[^cn-47]: @ai:subagent | 2026-04-02 | ins | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Duplicate Level 0 construct table after splice; redundant and malformed flow."

[^cn-48]: @ai:subagent | 2026-04-02 | ins | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Inherited duplicate table row edits after corrupted splice; noisy."

[^cn-49]: @ai:subagent | 2026-04-02 | del | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same duplicate-table cleanup noise."

[^cn-50]: @ai:subagent | 2026-04-02 | del | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same duplicate-table cleanup noise."

[^cn-51]: @ai:subagent | 2026-04-02 | sub | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same duplicate-table cleanup noise."

[^cn-52]: @ai:subagent | 2026-04-02 | sub | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Same duplicate-table cleanup noise."

[^cn-53]: @ai:subagent | 2026-04-02 | hig | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Duplicate highlight proposals in corrupted second table."

[^cn-54]: @ai:subagent | 2026-04-02 | hig | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Duplicate highlight proposals in corrupted second table."

[^cn-55]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Empty comment in duplicate table; noisy."

[^cn-56]: @ai:subagent | 2026-04-02 | com | rejected
    rejected: @ai:subagent 2026-04-02T02:20:30Z "Empty comment in duplicate table; noisy."

[^cn-57]: @ai:subagent | 2026-04-02 | sub | accepted
    approved: @ai:subagent 2026-04-02T02:20:30Z "Coherent complete-example narrative: REST→GraphQL in compacted story matches intended example direction; not garbled."

[^cn-58]: @ai:subagent | 2026-04-02 | sub | accepted
    approved: @ai:subagent 2026-04-02T02:20:30Z "Preserves contextual embedding pedagogy (Protocol o→new o verview); text remains coherent."

[^cn-59]: @ai:subagent | 2026-04-02 | del | accepted
    approved: @ai:subagent 2026-04-02T02:20:30Z "Removes spurious `{--text--}`-style noise from explanatory prose near anchoring example; cleanup aligns with readable spec."

[^cn-60]: @ai:subagent | 2026-04-02 | sub | accepted
    approved: @ai:subagent 2026-04-02T02:20:30Z "Coherent complete-example line: REST→GraphQL in three-authors narrative; matches surrounding resolved-thread example."

[^cn-61]: @ai:subagent | 2026-04-02 | hig | accepted
    approved: @ai:subagent 2026-04-02T02:20:30Z "Highlight on rate limit in complete example is coherent and matches open-thread scenario; not malformed."