...
Every edit is attributed, every decision recorded. The file carries its own deliberation history[^cn-1.1] — not in a sidebar or a separate system, but in the text itself. Anyone reading it sees not just what the document says, but the reasoning that shaped it.

---

**For humans** — a VS Code extension with the workflow you know from Word and Google Docs. Accept, reject, comment, navigate.
{++The comment panel keeps discussion next to the text — no context-switching to review. ++}[^cn-1.2]
...
{++**For teams** — shared review workflows with approval chains, amendment history, and compaction. ++}[^cn-3]
