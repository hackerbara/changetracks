...
{--![](02-editing-example_media/7bf4b3672eb5431bc7fe4330e9c6237948fd6493.png)--}[^cn-43]
...
{++![](02-editing-example_media/7bf4b3672eb5431bc7fe4330e9c6237948fd6493.png)++}[^cn-61]
...
{--![](02-editing-example_media/39147bb8e8058267e9a1c5e85296161e951acca4.png)--}[^cn-67]
...
- **Agent reads file state** {~~–~>–~~}[^cn-70] The agent reads the current file state through the MCP read_tracked_file tool, which returns every line with its LINE:HASH coordinate. The agent receives the full document context including pending changes, author metadata, and review status for each {--tracked--}[^cn-71] {~~operation~>tracked operation~~}[^cn-72].
- **Parser interprets intent** {++-++}[^cn-73] {-----}[^cn-74] {~~The CriticMarkup parser~>The CriticMarkup parser~~}[^cn-75] processes the request and identifies the appropriate editing action from the agent instructions through three stages:

1. **Change Detection:** {++–++}[^cn-76] {--The edit boundary engine detects keystroke sequences and cursor movements, segmenting them into discrete change operations for the tracking pipeline (--}[^cn-77]supporting{~~)~>thousands of~~}[^cn-78] {--concurrent--}[^cn-79] editing sessions

2. **Intent Understanding:** {++–++}[^cn-80] {--The footnote-native parser maps recognized text to editing intents such as insert, delete, substitute, highlight, and comment operations with full author (--}[^cn-81]attribution{--)--}[^cn-82]{--,--}[^cn-83] now enhanced with {~~LLM-powered contextual intent classification~>LLM-powered contextual intent classification~~}[^cn-84] for faster, description-based intent classification

3. **Response Generation:** {++–++}[^cn-85] For straightforward requests, {~~the MCP tool server~>the MCP tool server~~}[^cn-86] generates a direct response using its built-in propose_change fulfillment. For complex editing tasks requiring document context, {~~the system~>the system~~}[^cn-87] routes to large language models (such as Claude) {++for nuanced revision proposals++}[^cn-88] that respect existing tracked changes{++.++}[^cn-89]

{--This architecture supports multi-turn conversations with contextual awareness, so an agent can reference earlier edits in the same session and build on previous review decisions.--}[^cn-90]

- **Response flows back** {++-++}[^cn-91] {-----}[^cn-92] The MCP tool server formulates the edit proposal and routes it through the CriticMarkup parser for validation and serialization. The response includes fully attributed change operations with LINE:HASH coordinates. {~~The parser~>The parser~~}[^cn-93] {~~supports~>supports the~~}[^cn-94] {~~six-level matching cascade~>six-level matching cascade~~}[^cn-95] for fine-grained control over {++anchor resolution, whitespace normalization, and settled-text fallback (++}[^cn-96]STF{++)++}[^cn-97] in ambiguous contexts. The response flows back through the connector to the editor {~~via~>using~~}[^cn-98] the CriticMarkup protocol.
- **Escalation when needed** {++-++}[^cn-99] {-----}[^cn-100] If the virtual agent {~~cannot~>can't~~}[^cn-101] resolve the editing request — for example, when a proposed substitution conflicts with a pending change from another author — the system escalates to a human reviewer. The escalation preserves the full dialog{--ue--}[^cn-102] context so the reviewer can pick up without repeating steps.

{++This architecture supports multiturn conversations with contextual awareness, allowing agents to reference earlier edits and build on previous review decisions within the same session.++}[^cn-103]

{++The agent editing flow is illustrated in the following flow diagram.++}[^cn-104]

{++![](02-editing-example_media/39147bb8e8058267e9a1c5e85296161e951acca4.png)++}[^cn-105]

{++*Figure 2: ChangeDown agent editing environment and review integration call flow*++}[^cn-106]

{~~How t~>T~~}[^cn-107]his solution integrates with your editing environment {++in the following ways for collaborative workflows.++}[^cn-108]

- **CriticMarkup services:** {++–++}[^cn-109] The CriticMarkup parser handles {--change detection and edit-boundary segmentation, while the footnote-native parser provides structured metadata extraction for attributed review (--}[^cn-110]responses{--)--}[^cn-111] and {--agentic developer tools provide the orchestration framework for agent-driven editing workflows with full LINE:HASH (--}[^cn-112]addressing{--)--}[^cn-113]. The parser resolves ambiguous anchors using {~~the six-level matching cascade~>the matching cascade~~}[^cn-114].
- **Agentic developer tools:** {++–++}[^cn-115] The MCP tool server provides the orchestration framework for agent-driven editing workflows with full LINE:HASH addressing.
- **Generative AI with** {++**large language**++}[^cn-116] **models:** {++–++}[^cn-117] Large language models power context-aware edit generation — {~~the model reads full document state~>the model reads full document state~~}[^cn-118] via read_tracked_file before proposing {~~attributed changes~>attributed changes,~~}[^cn-119] {--—--}[^cn-120]enabling responses that go beyond scripted intents.

**Why this design matters**

- **Your files, your rules.** The ChangeDown {~~extension~>extension~~}[^cn-121] runs entirely in your own editor, giving you full control over customization, data retention, and compliance.
...
- **Deploy where you need.** Available as an open source extension supporting global teams{++, with local-first++}[^cn-122] processing and no cloud dependency for all supported editing operations.[^cn-123]

**Use cases across collaborative workflows**

{++Organizations++}[^cn-124] {++across industries can find benefits in pursuing tracked, AI-assisted document management solutions.++}[^cn-125]

{--**Healthcare:**--}[^cn-126]

{++Healthcare organizations can++}[^cn-127] enhance clinical documentation with intelligent template management, automated discharge summary drafting, and smart protocol routing that assigns revisions to the right specialist while maintaining comprehensive audit trails of every clinical document {--change--}[^cn-128] revision.

{--**Finance:**--}[^cn-129]

{++Finance organizations can++}[^cn-130] improve document self-service in situations like regulatory filing preparation, contract revision tracking, and compliance report generation, all while maintaining the strict change attribution and editorial audit trails that financial regulators require for audit readiness and operational {~~transparency~>transparency~~}[^cn-131].[^cn-132]

{--**Government:**--}[^cn-133]

Government agencies can streamline policy document workflows with full change attribution, supporting inter-agency review processes with tracked revisions for benefit enrollment guides, permit application forms, and public records documentation.[^cn-134]

**Workflow benefits**

{--**Editorial workflow automation:**--}[^cn-135]

{----}[^cn-136]Tracked AI editing transforms routine document revisions{++, including++}[^cn-137] {--—--}[^cn-138]content updates, structural reorganization, and style corrections, into streamlined self-service experiences. The integration delivers consistent editorial quality around the clock and scales to handle revision volume spikes without adding headcount or compromising review quality. The result{~~:~>is~~}[^cn-139] consistent, quality support available {~~24/7~>around the clock~~}[^cn-140] without proportional staffing costs.

| | **Step** | **Traditional** {++p++}[^cn-141]{--P--}[^cn-142]rocess **|** With {--i--}[^cn-143]{++ChangeDown integration++}[^cn-144] {++i++}[^cn-145]{--I--}[^cn-146]ntegration **|**
...
{--The CriticMarkup matching cascade's--}[^cn-159] {~~contextual editing~>contextual editing capabilities in the CriticMarkup matching cascade~~}[^cn-160] enables genuine contextual editing experiences. The system recognizes returning authors{~~,~>;~~}[^cn-161] it references their change history{~~:~>. It~~}[^cn-162] recogniz{++es++}[^cn-163]{--ing--}[^cn-164] returning authors, referenc{++es++}[^cn-165]{--ing--}[^cn-166] their change history, and anticipat{++es++}[^cn-167]{--ing--}[^cn-168] revision patterns based on previous edits. This intelligent, empathetic approach builds loyalty by making every author feel known, not processed.

{--Summary/--}[^cn-169]Conclusion

In today's collaborative editing deployments, organizations face a definitive challenge{++.++}[^cn-170]{~~:~>The need to address h~~}[^cn-171]{--H--}[^cn-172]ow to scale AI-assisted document workflows without sacrificing the editorial accountability that reviewers and compliance teams require. Through the {~~open-source~>open source~~}[^cn-173] CriticMarkup connector, combined with the MCP tool server and VS Code extension, delivers a production-ready solution.

Organizations implementing this {~~Open-source~>o~~}[^cn-174]{++pen source++}[^cn-175] {~~integration~>CriticMarkup integration~~}[^cn-176] gain measurable advantages in editorial throughput{--,--}[^cn-177] review quality, and document integrity. This solution integrates with proven {~~CriticMarkup~>open source~~}[^cn-178] standards to improve every interaction and maintain context and intent across complete conversations{++,++}[^cn-179] {--that--}[^cn-180] improv{~~es~>ing~~}[^cn-181] your editorial experience.

Ready to transform your collaborative editing strategy? {++Connect with the ChangeDown++}[^cn-182] open source community {++to design a deployment architecture++}[^cn-183] tailored to your specific editorial workflow requirements.[^cn-184]
...
[^cn-78]: @editor | 2026-03-21 | sub | proposed

[^cn-79]: @editor | 2026-03-21 | del | proposed

[^cn-80]: @editor | 2026-03-21 | ins | proposed

[^cn-81]: @editor | 2026-03-21 | del | proposed

[^cn-82]: @editor | 2026-03-21 | del | proposed

[^cn-83]: @editor | 2026-03-21 | del | proposed

[^cn-84]: @editor | 2026-03-21 | sub | proposed

[^cn-85]: @editor | 2026-03-21 | ins | proposed

[^cn-86]: @editor | 2026-03-21 | sub | proposed

[^cn-87]: @editor | 2026-03-21 | sub | proposed

[^cn-88]: @editor | 2026-03-21 | ins | proposed

[^cn-89]: @editor | 2026-03-21 | ins | proposed

[^cn-90]: @editor | 2026-03-21 | del | proposed

[^cn-91]: @editor | 2026-03-21 | ins | proposed

[^cn-92]: @editor | 2026-03-21 | del | proposed

[^cn-93]: @editor | 2026-03-21 | sub | proposed

[^cn-94]: @editor | 2026-03-21 | sub | proposed

[^cn-95]: @editor | 2026-03-21 | sub | proposed

[^cn-96]: @editor | 2026-03-21 | ins | proposed

[^cn-97]: @editor | 2026-03-21 | ins | proposed

[^cn-98]: @editor | 2026-03-21 | sub | proposed

[^cn-99]: @editor | 2026-03-21 | ins | proposed

[^cn-100]: @editor | 2026-03-21 | del | proposed

[^cn-101]: @editor | 2026-03-21 | sub | proposed

[^cn-102]: @editor | 2026-03-21 | del | proposed

[^cn-103]: @editor | 2026-03-21 | ins | proposed
...
[^cn-107]: @editor | 2026-03-21 | sub | proposed

[^cn-108]: @editor | 2026-03-21 | ins | proposed

[^cn-109]: @editor | 2026-03-21 | ins | proposed

[^cn-110]: @editor | 2026-03-21 | del | proposed
...
[^cn-114]: @editor | 2026-03-21 | sub | proposed

[^cn-115]: @editor | 2026-03-21 | ins | proposed

[^cn-116]: @editor | 2026-03-21 | ins | proposed

[^cn-117]: @editor | 2026-03-21 | ins | proposed

[^cn-118]: @editor | 2026-03-21 | sub | proposed

[^cn-119]: @editor | 2026-03-21 | sub | proposed

[^cn-120]: @editor | 2026-03-21 | del | proposed

[^cn-121]: @editor | 2026-03-21 | sub | proposed

[^cn-122]: @editor | 2026-03-21 | ins | proposed

[^cn-123]: @editor | 2026-03-21 | comment | proposed
...
[^cn-124]: @editor | 2026-03-21 | ins | proposed

[^cn-125]: @editor | 2026-03-21 | ins | proposed

[^cn-126]: @editor | 2026-03-21 | del | proposed

[^cn-127]: @editor | 2026-03-21 | ins | proposed

[^cn-128]: @editor | 2026-03-21 | del | proposed

[^cn-129]: @editor | 2026-03-21 | del | proposed

[^cn-130]: @editor | 2026-03-21 | ins | proposed

[^cn-131]: @editor | 2026-03-21 | sub | proposed

[^cn-132]: @editor | 2026-03-21 | comment | proposed
    @editor 2026-03-21: **NEEDS ACTION:** I revised this section to prose. Please review for accuracy.

[^cn-133]: @editor | 2026-03-21 | del | proposed

[^cn-134]: @editor | 2026-03-21 | comment | proposed
    @editor 2026-03-21: **NEEDS ACTION:** Please revise to complete sentences

[^cn-135]: @editor | 2026-03-21 | del | proposed

[^cn-136]: @editor | 2026-03-21 | del | proposed

[^cn-137]: @editor | 2026-03-21 | ins | proposed

[^cn-138]: @editor | 2026-03-21 | del | proposed

[^cn-139]: @editor | 2026-03-21 | sub | proposed

[^cn-140]: @editor | 2026-03-21 | sub | proposed

[^cn-141]: @editor | 2026-03-21 | ins | proposed

[^cn-142]: @editor | 2026-03-21 | del | proposed

[^cn-143]: @editor | 2026-03-21 | del | proposed
    merge-detected: 1 original revisions

[^cn-144]: @editor | 2026-03-21 | ins | proposed
...
[^cn-161]: @editor | 2026-03-21 | sub | proposed

[^cn-162]: @editor | 2026-03-21 | sub | proposed

[^cn-163]: @editor | 2026-03-21 | ins | proposed

[^cn-164]: @editor | 2026-03-21 | del | proposed

[^cn-165]: @editor | 2026-03-21 | ins | proposed

[^cn-166]: @editor | 2026-03-21 | del | proposed

[^cn-167]: @editor | 2026-03-21 | ins | proposed

[^cn-168]: @editor | 2026-03-21 | del | proposed

[^cn-169]: @editor | 2026-03-21 | del | proposed

[^cn-170]: @editor | 2026-03-21 | ins | proposed

[^cn-171]: @editor | 2026-03-21 | sub | proposed

[^cn-172]: @editor | 2026-03-21 | del | proposed

[^cn-173]: @editor | 2026-03-21 | sub | proposed

[^cn-174]: @editor | 2026-03-21 | sub | proposed

[^cn-175]: @editor | 2026-03-21 | ins | proposed

[^cn-176]: @editor | 2026-03-21 | sub | proposed

[^cn-177]: @editor | 2026-03-21 | del | proposed

[^cn-178]: @editor | 2026-03-21 | sub | proposed

[^cn-179]: @editor | 2026-03-21 | ins | proposed

[^cn-180]: @editor | 2026-03-21 | del | proposed

[^cn-181]: @editor | 2026-03-21 | sub | proposed

[^cn-182]: @editor | 2026-03-21 | ins | proposed

[^cn-183]: @editor | 2026-03-21 | ins | proposed

[^cn-184]: @editor | 2026-03-21 | comment | proposed
