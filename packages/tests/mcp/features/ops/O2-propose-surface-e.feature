Feature: Propose changes via Surface E (decided view)
  As an AI agent using the decided view
  ...
  Scenario: Read decided view then propose with line:hash
  ...
  Scenario: Chained edits update hash state
    ...
    And I call read_tracked_file again with view = "decided"
  ...
  @wip
  Scenario: Decided view hides pending changes
